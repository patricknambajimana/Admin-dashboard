        ...
        <table className="w-full table-wrapper rounded-md p-3">
          ...
          <tbody className="divide-y">
            {User.map((user, index) => {
              ...
              const statusClass =
                user.status === "active"
                  ? "text-green-500 bg-green-700/20 rounded-full"
                  : "text-red-400 bg-red-400/20 rounded-full";

              return (
                <tr
                  key={index}
                  className="text-left  border-b border-gray-600/20">
                  <td className="flex items-center justify-start gap-2 p-3 whitespace-nowrap">
                    ...
                    <span className="flex flex-col gap-2 items-start">
                      <h2 className="text-xl font-bold capitalize px-2">
                        {user.username}
                      </h2>
                      <h2 className="text-1xl font-light capitalize px-2">
                        {user.email}
                      </h2>
                    </span>
                  </td>
                  ...
                  <td className="capitalize whitespace-nowrap">
                    <span className={`p-2 ${statusClass}`}>{user.status}</span>{" "}
                  </td>
                  <td className="text-sm font-extralight">
                    <span className="text-sm font-light p-2">
                      {user.lastlogin}
                    </span>
                  </td>
